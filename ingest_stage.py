...
def parse_fastq_name(name: str):
    m = re.match(r"(.+)_([12])\.fastq(\.gz)?$", name)
    if m:
        return m.group(1), int(m.group(2))
    m2 = re.match(r"(.+)\.fastq(\.gz)?$", name)
    if m2:
    ...
    if os.path.isdir(illum_root):
        for sample_dir in os.listdir(illum_root):
            ...
            if not os.path.isdir(sample_path):
                continue
            for fpath in Path(sample_path).rglob("*.fastq*"):
...
def ingest_inputs(inputs, project_root: str, platform: str):
    ...
    for item in inputs:
        ...
        if os.path.isdir(item):
            for p in Path(item).rglob("*.fastq*"):
                if "trimmed" in p.name or "nonhost" in p.name:
                    continue
                add_path(str(p))
            continue
        if os.path.isfile(item):
            if re.search(r"\.fastq(\.gz)?$", os.path.basename(item)):
                add_path(item)
            else:
                with open(item) as f:
                    for line in f:
