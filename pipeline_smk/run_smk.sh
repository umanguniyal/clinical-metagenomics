...
if [[ -n "${PROFILE_OVERRIDE}" ]]; then
  snakemake --use-conda --rerun-incomplete --keep-going --cores "${CORES}" all --config profile="${PROFILE_OVERRIDE}"
else
  snakemake --use-conda --rerun-incomplete --keep-going --cores "${CORES}" all
fi
