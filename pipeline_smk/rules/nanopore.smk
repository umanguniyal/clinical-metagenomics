...
def get_control_json_nanopore(wildcards):
    if wildcards.sample + "_control" in SAMPLES_NANOPORE:
        return os.path.join(RESULTS_DIR, "nanopore", wildcards.sample + "_control", "report", f"report_{PROFILE}.json")
    return []
...
rule nanopore_clinician_report:
    input:
        ...
        tbprofiler_json = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "taxonomy", "tbprofiler", "results", "{sample}.results.json"),
        checkv_tsv = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "binning_checkv", "quality_summary.tsv"),
        busco_txt = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "binning_busco", "short_summary.txt"),
        control_json = get_control_json_nanopore
    output:
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "nanopore_clinician_report.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "nanopore_clinician_report.txt")
    conda:
...
rule nanopore_bioinformatician_report:
    input:
        ...
        kma_res = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "amr", "kma_resfinder", "kma.res"),
        kma_tsv = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "amr", "kma_resfinder", "kma.tsv"),
        prokka_json = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "bin_annotations", "prokka_stats.json"),
        stats = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "host_removed", "host_stats.json"),
        tbprofiler_json = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "taxonomy", "tbprofiler", "results", "{sample}.results.json"),
        checkv_tsv = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "binning_checkv", "quality_summary.tsv"),
        busco_txt = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "binning_busco", "short_summary.txt"),
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "nanopore_bioinformatician_report.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "nanopore_bioinformatician_report.txt")
    conda:
        os.path.join(ENVS_DIR, "base_utils.yaml")
    threads: 1
    shell:
