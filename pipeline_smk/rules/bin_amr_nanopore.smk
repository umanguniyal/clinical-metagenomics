...
rule nanopore_bin_amr_map:
    input:
        ...
        bin_tax_dir=os.path.join(RESULTS_DIR, "nanopore", "{sample}", "bin_taxonomy"),
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "nanopore_bin_amr_map.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "nanopore_bin_amr_map.txt")
    conda:
