...
rule illumina_contig_amr:
    ...
    log:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "logs", "illumina_contig_amr.log")
    benchmark:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "benchmarks", "illumina_contig_amr.txt")
    conda:
    ...
    shell:
        ...
        python3 scripts/run_contig_amr.py \
          ...
          --mode-file {input.amr_mode} \
          --outdir $(dirname {output.abricate_card}) > {log} 2>&1
        """
...
rule illumina_kma_resfinder:
    ...
    log:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "logs", "illumina_kma_resfinder.log")
    benchmark:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "benchmarks", "illumina_kma_resfinder.txt")
    conda:
    ...
    shell:
        ...
        python3 scripts/run_kma_resfinder.py \
          ...
          --platform illumina \
          --outdir $(dirname {output.kma_res}) --threads {threads} > {log} 2>&1
        """
