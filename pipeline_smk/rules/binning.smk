...
rule illumina_binning_bam:
    input:
        ...
        reads1 = os.path.join(RESULTS_DIR, "illumina", "{sample}", "host_removed", "nonhost_R1.fastq.gz"),
        reads2 = os.path.join(RESULTS_DIR, "illumina", "{sample}", "host_removed", "nonhost_R2.fastq.gz")
    output:
    ...
    log:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "logs", "illumina_binning_bam.log")
    benchmark:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "benchmarks", "illumina_binning_bam.txt")
    conda:
    ...
    shell:
        ...
        # Build bowtie2 index
        TMPIDX=$(mktemp -d)
        bowtie2-build {input.contigs} $TMPIDX/idx --threads {threads} 2>> {log}
        
        # Map reads and create sorted BAM
        bowtie2 -x $TMPIDX/idx -1 {input.reads1} -2 {input.reads2} --threads {threads} -q 2>> {log} | \
...
rule nanopore_binning_bam:
    input:
        ...
        reads = os.path.join(RESULTS_DIR, "nanopore", "{sample}", "host_removed", "nonhost_nanopore.fastq.gz")
    output:
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "nanopore_binning_bam.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "nanopore_binning_bam.txt")
    conda:
...
rule illumina_binning_depth:
    ...
    log:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "logs", "illumina_binning_depth.log")
    benchmark:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "benchmarks", "illumina_binning_depth.txt")
    conda:
...
rule nanopore_binning_depth:
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "nanopore_binning_depth.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "nanopore_binning_depth.txt")
    conda:
        os.path.join(ENVS_DIR, "metabat2.yaml")
    threads: 1
    shell:
...
rule nanopore_estimate_contig_mean:
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "nanopore_estimate_contig_mean.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "nanopore_estimate_contig_mean.txt")
    conda:
...
rule run_metabat2_illumina:
    ...
    log:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "logs", "run_metabat2_illumina.log")
    benchmark:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "benchmarks", "run_metabat2_illumina.txt")
    conda:
        os.path.join(ENVS_DIR, "metabat2.yaml")
    threads: THREADS
    resources:
    ...
    shell:
        ...
        if [[ "{wildcards.sample}" == *_control ]] && grep -q "LOW_COV" {input.mode}; then
          echo "Control sample with LOW_COV. Skipping MetaBAT2."
...
rule run_metabat2_nanopore:
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "run_metabat2_nanopore.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "run_metabat2_nanopore.txt")
    conda:
        os.path.join(ENVS_DIR, "metabat2.yaml")
    threads: THREADS
    resources:
...
rule run_mob_recon_illumina:
    ...
    log:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "logs", "run_mob_recon_illumina.log")
    benchmark:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "benchmarks", "run_mob_recon_illumina.txt")
    conda:
...
rule run_mob_recon_nanopore:
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "run_mob_recon_nanopore.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "run_mob_recon_nanopore.txt")
    conda:
        os.path.join(ENVS_DIR, "mob_suite.yaml")
    threads: THREADS
    resources:
...
rule illumina_classify_bins:
    ...
    log:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "logs", "illumina_classify_bins.log")
    benchmark:
        os.path.join(RESULTS_DIR, "illumina", "{sample}", "benchmarks", "illumina_classify_bins.txt")
    conda:
...
rule nanopore_classify_bins:
    ...
    log:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "logs", "nanopore_classify_bins.log")
    benchmark:
        os.path.join(RESULTS_DIR, "nanopore", "{sample}", "benchmarks", "nanopore_classify_bins.txt")
    conda:
        os.path.join(ENVS_DIR, "centrifuge.yaml")
    threads: THREADS
    shell:
