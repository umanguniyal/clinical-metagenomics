...
rule estimate_read_length:
    ...
    log:
        os.path.join(RESULTS_DIR, "{platform}", "{sample}", "logs", "estimate_read_length.log")
    benchmark:
        os.path.join(RESULTS_DIR, "{platform}", "{sample}", "benchmarks", "estimate_read_length.txt")
    conda:
...
if PLATFORM == "illumina":
    rule bracken:
        ...
        log:
            os.path.join(RESULTS_DIR, "{platform}", "{sample}", "logs", "bracken.log")
        benchmark:
            os.path.join(RESULTS_DIR, "{platform}", "{sample}", "benchmarks", "bracken.txt")
        conda:
        ...
        shell:
            ...
            python3 scripts/run_bracken.py \
              ...
              --level "{config[taxonomy][bracken_level]}" > {log} 2>&1
            """
