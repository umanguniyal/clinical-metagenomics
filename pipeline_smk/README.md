...
7. **CheckM2 Database**: `uniref100.KO.1.dmnd` — this pipeline uses CheckM2 <1.1 (v1.0.x). Ensure the database was downloaded with the matching version: `checkm2 database --download --path databases/checkm2_db/`
8. **CheckV Database**: Required for viral bin quality assessment. Download via `checkv download_database ./checkv-db-v1.5` and set the path in config.
9. **BUSCO Lineages**: Required for eukaryotic/prokaryotic completeness checking. Run `busco --download_path ./busco_downloads --download bacteria_odb10` (and other required lineages) or allow auto-download if internet is available.
