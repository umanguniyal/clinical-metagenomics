...
def prepare_context(report):
    ...
    ctx["merged_taxonomy"] = merged_tax[:20]
    ...
    if "common" in summary or "disjoint" in summary:
        ctx["merged_amr"] = {
            "common": [h for h in summary.get("common", []) if float(h.get("median_coverage", 0)) >= 50.0],
            "disjoint": [h for h in summary.get("disjoint", []) if float(h.get("coverage", 0)) >= 50.0]
        }
    else:
    ...
    ctx["kma_hits"] = [h for h in kma_info.get("hits", []) if float(h.get("coverage_pct", 0)) >= 50.0][:20]
    ...
    # Binned AMR
    bin_amr_data = report.get("bin_amr", {})
    if "bin_amr_hits" in bin_amr_data:
        bin_amr_data["bin_amr_hits"] = [
            h for h in bin_amr_data["bin_amr_hits"]
            if h.get("high_confidence") or float(h.get("identity", 0) or 0) >= 80.0
        ]
    ...
    # Prokka stats
    prokka_data = report.get("prokka", {})
    if "bins" in prokka_data:
        prokka_data["bins"] = {
            k: v for k, v in prokka_data["bins"].items()
            if "unbinned" not in k.lower() and "tooshort" not in k.lower()
        }
    ctx["prokka"] = prokka_data
