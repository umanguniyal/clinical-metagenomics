...
def get_sid_for_input(item: str) -> str:
    if is_accession(item):
        return item
    if is_url(item) or os.path.isfile(item):
        bn = url_basename(item) if is_url(item) else os.path.basename(item)
        sid, _ = parse_fastq_name(bn)
        if not sid:
            sid = re.sub(r"\.(?:fastq|fq)(?:\.gz)?$", "", bn)
        return sid
    if os.path.isdir(item):
        return os.path.basename(item.rstrip('/'))
    return None
...
def scan_staged(project_root: str):
    ...
    for sid, info in candidates.items():
        ...
        if 1 in illum_files and 2 in illum_files:
            r1 = os.path.abspath(normalize_fastq_gz(illum_files[1]))
            r2 = os.path.abspath(normalize_fastq_gz(illum_files[2]))
            rows.append(("illumina", sid, r1, r2))
            ...
            if src:
                fq = os.path.abspath(normalize_fastq_gz(src))
                rows.append(("nanopore", sid, fq, ""))
...
def main():
    ...
    ap.add_argument("--batch-control", help="Control input for all samples in batch")
    ap.add_argument("--control-pairs", nargs="*", default=[], help="List of item::ctrl pairs")
    ap.add_argument("--append", action="store_true", help="Append to manifest instead of overwriting")
    args = ap.parse_args()
    ...
    if args.inputs and len(args.inputs) > 0:
        ...
        # Handle controls
        if args.batch_control:
            ctrl_rows = ingest_inputs([args.batch_control], project_root, args.platform)
            if ctrl_rows:
                plat, _, c_r1, c_r2 = ctrl_rows[0]
                patient_sids = [r[1] for r in rows if not r[1].endswith("_control")]
                for sid in patient_sids:
                    rows.append((plat, f"{sid}_control", c_r1, c_r2))
                    
        if args.control_pairs:
            for pair in args.control_pairs:
                if "::" not in pair: continue
                it, ctrl = pair.split("::", 1)
                patient_sid = get_sid_for_input(it)
                if not patient_sid: continue
                c_rows = ingest_inputs([ctrl], project_root, args.platform)
                if c_rows:
                    plat, _, c_r1, c_r2 = c_rows[0]
                    rows.append((plat, f"{patient_sid}_control", c_r1, c_r2))
    else:
        rows = scan_staged(project_root)
    mode = "a" if args.append else "w"
    with open(args.out_manifest, mode) as out:
        if mode == "w" or os.stat(args.out_manifest).st_size == 0:
