...
def extract_cohort_features(json_files):
    ...
    for path in json_files:
        ...
        # Get ML scores and labels from predictions.csv
        preds_path = os.path.join(os.path.dirname(os.path.dirname(path)), "ml_results", "predictions.csv")
        if os.path.exists(preds_path):
            preds = pd.read_csv(preds_path)
            score_map = dict(zip(preds["organism"], preds["pathogen_score"]))
            label_map = dict(zip(preds["organism"], preds["heuristic_label"]))
            df["pathogen_score"] = df["organism"].apply(lambda x: score_map.get(x, 0.0))
            df["heuristic_label"] = df["organism"].apply(lambda x: label_map.get(x, 0))
        else:
            df["pathogen_score"] = 0.0
            df["heuristic_label"] = 0
            
        rows.append(df)
        
        # Get feature importances
        fi_path = os.path.join(os.path.dirname(os.path.dirname(path)), "ml_results", "feature_importances.csv")
        if os.path.exists(fi_path):
            fi_df = pd.read_csv(fi_path)
            fi_df["sample_id"] = sample_id
    ...
    return pd.concat(rows, ignore_index=True), pd.DataFrame(amr_associations), pd.DataFrame(concordances), pd.concat(feature_importances, ignore_index=True) if feature_importances else pd.DataFrame()

def plot_cohort_pca(df, outpath):
    if len(df) < 2:
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.text(0.5, 0.5, "No Data Available\n(Need >= 2 samples for PCA)", ha='center', va='center', fontsize=12, color='gray')
        ax.axis('off')
        _save(fig, outpath)
        return
...
def plot_species_amr_matrix(assoc_df, outpath):
    if assoc_df.empty:
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.text(0.5, 0.5, "No Data Available\n(No AMR associations)", ha='center', va='center', fontsize=12, color='gray')
        ax.axis('off')
        _save(fig, outpath)
        return
...
def plot_checkm2_vs_prokka(df, outpath):
    ...
    if valid_df.empty:
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.text(0.5, 0.5, "No Prokka Data Available", ha='center', va='center', fontsize=12, color='gray')
        ax.axis('off')
        _save(fig, outpath)
        return
    ...
    ax.set_title("Genome Assembly Quality vs. Annotation Coding Density")
    ax.grid(True, linestyle="--", alpha=0.5)
    
    _save(fig, outpath)


def plot_feature_stability(fi_df, outpath):
    if fi_df.empty: return
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.boxplot(data=fi_df, x="importance", y="feature", ax=ax, palette="viridis", showfliers=False)
    sns.stripplot(data=fi_df, x="importance", y="feature", ax=ax, color="black", alpha=0.3, size=3)
    ax.set_title("Feature Importance Stability Across Cohort")
    ax.set_xlabel("Importance Score")
    ax.set_ylabel("")
    _save(fig, outpath)

def plot_calibration_curve(df, outpath):
    if df.empty or "heuristic_label" not in df.columns: return
    from sklearn.calibration import calibration_curve
    labels = df["heuristic_label"].values
    scores = df["pathogen_score"].values
    if len(np.unique(labels)) < 2: return
    prob_true, prob_pred = calibration_curve(labels, scores, n_bins=10)
    
    fig, ax = plt.subplots(figsize=(6, 6))
    ax.plot(prob_pred, prob_true, marker='o', linewidth=2, color="#2980b9", label="RF Model Calibration")
    ax.plot([0,1], [0,1], "k--", label="Perfectly Calibrated")
    ax.set_title("Pathogen Score Reliability Diagram")
    ax.set_xlabel("Predicted Probability")
    ax.set_ylabel("Fraction of True Pathogens")
    ax.legend()
    _save(fig, outpath)
...
def main():
    ...
    df, assoc_df, conc_df, fi_df = extract_cohort_features(args.reports)
    if df.empty:
        ...
        plot_feature_stability(fi_df, os.path.join(args.outdir, "cohort_feature_stability.png"))
        plot_calibration_curve(df, os.path.join(args.outdir, "cohort_calibration_curve.png"))
        plot_checkm2_vs_prokka(df, os.path.join(args.outdir, "cohort_checkm2_vs_prokka.png"))
        sys.exit(0)
