...
def extract_features(report: dict) -> pd.DataFrame:
    ...
    for sp in top_species:
        ...
        if matched_bins:
            ...
            p_stats = [prokka.get(b, {}) for b in matched_bins if b in prokka]
            if p_stats:
                cds_count = sum(s.get("cds_count", 0) for s in p_stats)
                trna_count = sum(s.get("trna_count", 0) for s in p_stats)
                rrna_count = sum(s.get("rrna_count", 0) for s in p_stats)
                hyp_count = sum(s.get("hypothetical_count", 0) for s in p_stats)
                
                mean_gene_density = float(np.mean([s.get("gene_density", 0.0) for s in p_stats]))
                mean_coding_density = float(np.mean([s.get("coding_density_pct", 0.0) for s in p_stats]))
                
                if cds_count > 0:
                    hyp_ratio = round(hyp_count / cds_count, 4)
        ...
        rows.append({
            ...
            "cds_count":          cds_count,
            "gene_density":       mean_gene_density,
            "trna_count":         trna_count,
            "rrna_count":         rrna_count,
            "hypothetical_ratio": hyp_ratio,
            "coding_density_pct": mean_coding_density,
        })
...
def train_and_predict(df: pd.DataFrame, labels: np.ndarray, model_type: str = "rf") -> tuple[np.ndarray, object, pd.Series]:
    ...
    # When all training labels are the same class (e.g. single-sample run where
    # every organism is labelled "non-pathogen"), predict_proba returns a matrix
    # with only 1 column.  Guard against the IndexError.
    proba_matrix = model.predict_proba(X_scaled)
    if proba_matrix.shape[1] >= 2:
        proba = proba_matrix[:, 1]
    else:
        # Only one class was seen — assign a uniform low/high score depending
        # on which class is present (0 → all non-pathogen, 1 → all pathogen).
        sole_class = model.classes_[0]
        proba = np.full(len(labels), float(sole_class))
...
FEATURE_LABELS = { "rel_abundance": "Relative Abundance (%)", "abs_reads": "Total Reads", "completeness": "Genome Completeness (%)", "contamination": "Genome Contamination (%)", "kma_n_hits": "KMA Fallback: AMR Gene Count", "kma_mean_identity": "KMA Fallback: Mean Identity (%)", "kma_mean_depth": "KMA Fallback: Mean Depth", "kma_mean_coverage_pct": "KMA Fallback: Mean Coverage (%)", "sample_alpha_diversity": "Sample Diversity (Shannon H)", "mean_sample_coverage": "Overall Sample Coverage" }
...
def plot_organism_quality_heatmap(df: pd.DataFrame, outpath: str):
    cols = ["completeness", "contamination", "rel_abundance"]; sub = df[["organism"] + cols].set_index("organism"); scaled = StandardScaler().fit_transform(sub.values.astype(float)); fig, ax = plt.subplots(figsize=(8, max(4, len(sub) * 0.4))); im = ax.imshow(scaled, aspect="equal", cmap="YlOrRd"); plt.colorbar(im, ax=ax, label="Normalized Value", fraction=0.046, pad=0.04); ax.set_xticks(range(len(cols))); ax.set_xticklabels([_get_lbl(c) for c in cols], rotation=45, ha="right", fontsize=9); ax.set_yticks(range(len(sub))); ax.set_yticklabels([idx[:35] for idx in sub.index], fontsize=8); ax.set_title("Organism Quality (Normalized)"); _save(fig, outpath)
...
def plot_checkm2_vs_prokka(df, outpath: str):
    if df.empty or "coding_density_pct" not in df.columns or "completeness" not in df.columns:
        plot_empty(outpath, "No Prokka data")
        return
        
    valid_df = df[df["coding_density_pct"] > 0]
    if valid_df.empty:
        plot_empty(outpath, "No Prokka data for bins")
        return

    fig, ax = plt.subplots(figsize=(8, 6))
    
    sc = ax.scatter(valid_df["completeness"], valid_df["coding_density_pct"], 
                    c=valid_df["hypothetical_ratio"], cmap="viridis", 
                    s=50, edgecolors="k", alpha=0.8)
                    
    cbar = plt.colorbar(sc, ax=ax)
    cbar.set_label("Hypothetical Protein Ratio", rotation=270, labelpad=15)
    
    # Label each point with the organism name
    for i, row in valid_df.iterrows():
        ax.annotate(row["organism"][:15], (row["completeness"], row["coding_density_pct"]), 
                    xytext=(5, 5), textcoords="offset points", fontsize=8, alpha=0.7)
    
    ax.set_xlabel("CheckM2 Completeness (%)")
    ax.set_ylabel("Prokka Coding Density (%)")
    ax.set_title("Organism Bin Quality vs. Annotation Coding Density")
...
def main():
    ...
    importances.to_csv(os.path.join(args.outdir, "feature_importances.csv"), header=["importance"], index_label="feature")
    ...
    plot_organism_quality_heatmap(df, os.path.join(args.outdir, "organism_quality_heatmap.png"))
    ...
    plot_checkm2_vs_prokka(df, os.path.join(args.outdir, "checkm2_vs_prokka.png"))
