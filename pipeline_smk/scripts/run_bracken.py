...
def main():
    ...
    # Snap readlen to closest available k-mer size to avoid bracken silently failing
    closest_kmer = min(available_kmers, key=lambda k: abs(k - readlen))
    ...
    print(f"Using snapped k-mer size: {closest_kmer}bp", file=sys.stderr)
    readlen = closest_kmer
