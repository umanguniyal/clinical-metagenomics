    ...
    with open(report_path) as f:
        for line in f:
            ...
            if len(parts) >= 6 and parts[3].strip() == "S":
                try:
                    name = parts[5].strip()
                    if "homo sapiens" in name.lower(): continue
                    species.append({
                        "name": name,
...
def parse_amrfinder_tsv(path):
    ...
    for ln in lines[1:]:
        ...
        if not contig and len(fields) > 1:
            contig = fields[1].split()[0]
        identity = get(fields, "% identity to reference", "0")
        coverage = get(fields, "% coverage of reference", "0")
...
def main():
    ...
    ap.add_argument("--prokka_json", default="")
    ap.add_argument("--tbprofiler_json", default="")
    ap.add_argument("--checkv_tsv", default="")
    ap.add_argument("--busco_txt", default="")
    ...
    checkv_data = parse_checkv(args.checkv_tsv) if hasattr(args, 'checkv_tsv') else []
    busco_data = parse_busco(args.busco_txt) if hasattr(args, 'busco_txt') else ""
    ...
    if args.prokka_json and os.path.exists(args.prokka_json):
        try:
            with open(args.prokka_json) as f:
                report["prokka"] = json.load(f)
        except:
            pass

    if args.tbprofiler_json and os.path.exists(args.tbprofiler_json):
        try:
            with open(args.tbprofiler_json) as f:
                tb_data = json.load(f)
                if tb_data and tb_data != {}:
                    report["tbprofiler"] = tb_data
        except:
            pass
