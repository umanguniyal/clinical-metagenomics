...
def main():
    ...
    if args.viral_xlsx and os.path.exists(args.viral_xlsx):
        try:
            ...
            for col in ["ICTV species", "Common name"]:
                if col in vdf.columns:
                    bartlett_species.extend(vdf[col].dropna().unique().tolist())
