...
def detect_resfinder_kma_prefix():
    for base in [Path(PROJECT_ROOT), Path(PROJECT_ROOT).parent]:
        for p in base.rglob("resfinder_db"):
            seqb = list(Path(p).rglob("all.seq.b")) or list(Path(p).rglob("resfinder_db_kma.seq.b"))
            if seqb:
                return str(seqb[0]).replace(".seq.b", "")
    return None

KRAKEN_DB = detect_kraken_db()
CENTRIFUGE_DB = detect_centrifuge_prefix()
HOST_MMI = detect_host_mmi()
HOST_BT2 = detect_host_bt2_prefix()
RESFINDER_DB = detect_resfinder_kma_prefix()

# PREFLIGHT CHECKS FOR PORTABILITY
import sys
missing_dbs = []
if not KRAKEN_DB:
    missing_dbs.append("- Kraken2 Database: Download using e.g. `kraken2-build --standard --db kraken_db`")
if not CENTRIFUGE_DB:
    missing_dbs.append("- Centrifuge Database: Download indices from Centrifuge website, e.g. `p_compressed+h+v`")
if PLATFORM == "nanopore" and not HOST_MMI:
    missing_dbs.append("- Human GRCh38 (Minimap2 index): Download GRCh38 fasta and run `minimap2 -d GRCh38.mmi GRCh38.fa`")
if PLATFORM == "illumina" and not HOST_BT2:
    missing_dbs.append("- Human GRCh38 (Bowtie2 index): Download GRCh38 and run `bowtie2-build GRCh38.fa GRCh38`")
if not RESFINDER_DB:
    missing_dbs.append("- ResFinder KMA Database: Clone bitbucket.org/genomicepidemiology/resfinder_db and index with `kma_index`")
...
def illumina_ml_targets():
    if PROFILE != "bioinformatician":
        return []
    return [os.path.join(RESULTS_DIR, "illumina", s, "ml_results", "predictions.csv") for s in SAMPLES_ILLUMINA]

def nanopore_ml_targets():
    if PROFILE != "bioinformatician":
        return []
    return [os.path.join(RESULTS_DIR, "nanopore", s, "ml_results", "predictions.csv") for s in SAMPLES_NANOPORE]
